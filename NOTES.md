...
- 2026-09-11 C012 T42：真实 L4/L5 操作矩阵（11对、双向）与既有锁顺序回归输出`6 passed in 23.01s`、exit 0；首个 replace→delete 并发500/串行404红测及锁后 Clip 存在性修复证据保留在`.work/c012/T42-test-red-replace-delete-20260911.*`。
...
- 2026-09-14 仓库整理：将本地 `.work/`、`.runtime/` 和 Vite/Ruff/TypeScript 缓存加入 Git 忽略规则；已跟踪的 `.work/c012/acceptance.py`、`lock-order.md`、正式测试、原始证据及项目媒体保留。缓存删除命令在执行前被工具安全层以 `blocked by policy` 拒绝，实际未删除文件；本轮仅提交忽略规则与事实记录，不改实现、不重跑测试。
- 2026-09-14 验收文件整理：按需求方授权将 `.work/c012/acceptance.py` 移至 `backend/scripts/c012_acceptance.py`、`lock-order.md` 移至 `openspec/archive/c012/lock-order.md`；修正脚本读取归档 spec 的路径和 CLI 帮助，更新当前文档入口，保留历史证据路径。`python -m py_compile backend/scripts/c012_acceptance.py`、`python -X utf8 backend/scripts/c012_acceptance.py --help`、`python -X utf8 backend/scripts/c012_acceptance.py verify-inputs` 均 exit 0，输入核对 `status=passed`；旧/新输入核对证据分别保存在 `.work/c012/path-relocation-20260914/verify-inputs-before.json` 与 `verify-inputs-after.json`。生产代码、正式测试、项目数据及视频未改，未运行模型或数据库验收。
