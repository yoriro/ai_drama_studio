...
- 四份正式模板内容已经由需求方在对应纵向 change 提供并确认:`script2assets`(C005)、`script2shots`(C006)、`zimage`(C007)、`minimaxh3`(C009)。模板是经设置页/API 维护、即时生效且不追溯的运行配置;仓库 migration 只放四个固定 key 的明确占位示例,不得把正式正文复制进 migration、生产代码常量或隐藏 system prompt。
- C012 发布验收必须在全新生产等价数据库迁移完成后,把上述四份既有正式内容作为部署输入,通过正式设置 API 全部安装。安装后及一次后端重启后分别 GET 回读,四个 key 必须齐全、正文与批准输入逐字一致且均不以 `[占位]` 开头;M6 全链路还必须实际消费四份模板。任一缺失、占位或不一致均阻塞发布,不得直接写库、复制旧库、依赖 E2E 中的临时人工编辑或使用 fallback。

C012 需求方最新批准：为修复M6分镜转视频提示词的动作遗漏，允许仅修订既有 `minimaxh3` 正式部署正文，方向为忠实逐镜转写、明确动作与时间、减少自由发挥；其他三个批准模板不变。五变量、输出schema、设置API部署/重启回读、R4缓存和不追溯语义不变；本轮新批准输入取代该key与C009历史正文逐字相同的要求，不增加版本机制。具体单次诊断与视频验收边界见C012 spec §6.3；失败保留，不以模板修改自动豁免M6视觉门槛。
...
7. **M6 E2E 与发布验收**:先在全新生产等价库通过正式设置 API 自动安装并逐字回读四份既有正式模板,后端重启后再次核对;随后完成一集剧本→资产→出图→分镜→两个片段视频全链路且实际消费四份模板,全程无后台手工干预;级联矩阵(3.3)全表回归;重启恢复(6.1)、trash 清理、竞态场景(3.2)逐项验证。

## 12. 前置依赖(开发前到位)

1. Z-Image、MiniMax H3 ref2v 两个 API 格式工作流 JSON + 节点绑定信息(§8),MiniMax 工作流须含可注入的 duration 输入(秒),并明确参考图输入数量与留空行为(不支持留空则提供可变参考数版本)。
2. 4 个提示词模板正式内容已分别在 C005-C009 提供并确认。占位示例只允许用于此前开发与干净测试库;C012 发布验收前必须按 §7 在全新生产等价库通过正式设置 API 部署全部四份,完成安装后与后端重启后的逐字回读,并在 M6 全链路中实际消费。
