...
4. **执行**：Luna 一次只完成一个指定 task，不得越出 spec；按 AGENTS.md「验收频率与证据复用」完成本 task 的受影响范围验收后才可勾选并提交。tasks 明列阶段完整回归节点；不默认每项跑完整 pytest。最终交付必须有覆盖最终受测实现的完整回归，同一受测输入可引用阶段证据；专项验收与未通过门槛不得省略。
