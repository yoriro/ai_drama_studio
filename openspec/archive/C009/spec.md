...
5. 复用 C008 的 closed rules 重新验证 R5 连续/独占与 R5a 至多一场景；创建后被编辑成双场景或跨场景时失败。当前 schema 以 `clip_shots.shot_id UNIQUE` 保证同一已提交状态中一个 Shot 至多属于一个 Clip，且生成入口的 source Shot 只来自目标 Clip 当前的 ClipShot 行，因此“目标 Clip 仍持有同一 Shot，同时另一 Clip 也持有它”不是当前生产 API/schema 可达的稳定输入状态。生产 `_other_clip_shot_ids` 防御查询保持不变；若它实际观察到占用仍按 R5 立即 failed，但验收不得通过删除约束、直接写非法数据或伪造查询结果制造该状态。
...
PyAV 只能在 `probe_clip_video_duration()` 实际探测完整媒体文件时按需导入；仅导入生产 `app.main`、clip API、`gen_clip_video` 或 `gen_asset_image` 入队模块时，`sys.modules` 中不得出现 `av`。本边界只延后原依赖的加载时机，不改变媒体解析库、异常、duration、worker 或文件事务语义，不得以 lazy proxy、兼容属性、mock、fallback 或替代解析器实现。
...
- **T22A 固定多进程导入探针**不新增仓库脚本或长期 driver：由 PowerShell 固定启动 20 个全新 Python 进程，每个只导入生产 `app.main`、clip API 与图像/视频入队模块，并逐个记录 PID、exit code 和 `av in sys.modules`。它与生产进程使用相同 Python 模块导入通路，能证明 API/入队导入边界不加载 PyAV且子进程正常退出；它不启动数据库、worker、外部服务或媒体探测，因此不能证明 MP4 解析。解析能力由既有 `test_c009_video_files.py` 生成真实 MP4 并调用生产 `probe_clip_video_duration()` 验证；该测试使用相同 PyAV/探测函数，但文件是测试内生成而非 Comfy 产物，故只能证明解析、错误和 duration 语义保持，不能替代 AC-21 的真实工作流视频证据。
- **R5 独占替代验收**不为生成入口新增“双占用”用例：该状态被初始 migration 与 ORM 的 `UNIQUE(clip_shots.shot_id)` 阻止，当前生产接口也没有在保留目标 Clip 的同时转移其 ClipShot 的通路；删除约束、直接造非法行或替换 `_other_clip_shot_ids` 查询结果只能证明人为状态，不能证明用户可达行为。替代验收为审计 ORM/migration 约束，并运行既有 C008 pure/API/跨进程用例，分别证明 closed rule、preview/create 422、两个创建请求争抢重叠 Shot 时精确一个完整胜方且无重复 ClipShot/孤儿行。C009 T24 仍自动验证生成入口可达的 R5 order 不连续以及完整 R5a/R10 立即 failed 矩阵。若后续 change 新增可在生成入队期间合法转移 ClipShot 的生产 API，届时必须新增该通路的并发验收；当前不预实现。
...
| AC-07 | [并发] | 在生成入队时分别制造可达的 R5 order 不连续、R5a 跨场/单镜双场、R10 各缺图原因，并带/不带实际 user_note mutation；另以两个独立创建请求争抢相同或部分重叠 Shot | HTTP/task/payload/Clip/queue/vLLM/Comfy、两个 create 响应、Clip/ClipShot/Slot 总数与唯一约束 | 生成入口各格均 HTTP 202；Task 直接 failed、progress0、started null、finished/error 非空、input_hash null且理由含规则/slot；从未 queued/claimed/外调；user_note 实际变化与 failed Task 同事务；只有 canceled/failed 历史时聚合符合 §8。重叠 create 精确一个完整 201，另一方结构化 422，每个 Shot 最多一条 ClipShot且无孤儿；不构造当前 schema/API 不可达的生成入口双占用状态 |
...
| AC-13 | [跨进程] | 固定启动 20 个全新 Python 进程并分别只导入生产应用、clip API、图像/视频入队模块；随后用既有视频文件测试生成合法 MP4、无 video stream、损坏、duration 缺失/NaN/inf/<=0 与 requested/actual 明显偏差 | 每个子进程 PID/exit code/`sys.modules`、`video_files.py` diff、parser、actual_duration、sha/temp/formal/DB/Task | 20 个 import-only 子进程均 exit 0 且 `av` 不在 `sys.modules`；生产 diff 只把同一 PyAV import 移入 `probe_clip_video_duration()`，既有测试调用该函数后仍用 PyAV 完成真实 MP4 探测；只有含 video stream 且 finite positive duration 成功，actual 保存探测值而非 requested 猜值，偏差不失败；所有非法文件 failed无 take/cache/formal；不依赖 PATH ffprobe或 fallback |
...
| AC-20 | [常规] | 打开并运行三个获窄授权 health 测试、分阶段演进的 C008 contract 测试、T13 演进的 C007 handler 注册测试、T20A 窄修正的 C009 生命周期测试、T22A 窄修正的 C009 视频文件测试、T25 窄演进的 C009 视频 handler 测试及所有新增测试，检查 git diff 与 TRACEABILITY | 测试断言、用例名、diff、追溯行 | health 三文件只把 hashes 精确集合演进为 zimage+minimaxh3；C008 contract 用例只按 T9/T13/T15 实际阶段向精确集合加入获授权 path/handler，既有 C008 行为/错误断言逐字保留且不改为子集/存在性；C007 注册用例只把视频 handler 不存在替换为生产 handler identity且保留资产 handler identity；C009 生命周期用例只把“所有事件两两不重叠”收窄为“vLLM 与 Comfy 跨组件活跃区间不重叠”，允许准确重命名 helper 及三处调用，保留区间合法性与其余全部断言且不使用时间容差；C009 视频文件用例只把 monkeypatch 目标从 `video_files.av` 改为已直接导入的 `av`，保留全部参数、错误消息与断言；C009 视频 handler 用例只把两个 LLM 失败与取消安全点的三个 `free_calls` 断言演进为全路径精确一次，保留其他断言；除这八个获授权文件外既有测试零修改；每个新增用例归属追溯行且无恒真/只非空弱断言 |
...
| AC-22 | [常规] | 运行隔离库完整 pytest、Alembic、前端 build、范围/文档/commit 审计并读取完成报告第5节 | 原始命令日志、git diff、TRACEABILITY、NOTES/DECISIONS、completion report | 全部计划测试与完整套件通过；Alembic零漂移；前端 build通过，前端 diff 精确只有 AC-24 两文件且无 C010 UI；`video_files.py` diff 只把 PyAV import 从模块顶层移入探测函数，获授权视频测试 diff 只有 monkeypatch 目标替换；c009目录只含spec/tasks；完成报告五节，第5节以“操作 → 观测值”覆盖设置页双 hash、真实主路径、R10 与 AC-21 真实工作流中断异常；TRACEABILITY 无待填；未验证项不宣称完成，收尾三项可核对 |
...
| AC-07 | R5 连续与独占；R5a 同场景；R10 缺图即失败；C009 立即 failed Task 与结构化错误；C009 复审可达 R5/R5a/R10 立即失败矩阵与 R5 独占替代证据 |
...
| AC-13 | C009 PyAV 按需加载与 Windows 子进程导入边界；C009 MP4 探测、文件/数据库事务与同步补偿 |
| AC-17 | C009 MP4 探测、文件/数据库事务与同步补偿 |
