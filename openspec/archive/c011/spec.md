...
### §10 当前状态澄清（2026-09-10）

本节以下带日期的审查/派发状态与“未完成”措辞保留为2026-09-09的历史诊断、授权和验收顺序记录，不再表示当前阻塞。后续T40–T45、T23、T24、T25以及T26/T27的实际回填已覆盖当前完成状态，T28也已完成文档与commit一致性收口；失败日志仍保留。B11–B15的触发/期望合同、证据边界和验收要求仍为规范内容，未因状态澄清而改变；当前后端完整回归复用`.work/c011/T39-astrafinal20260909loop-test.log`中的`348 passed in 239.87s`及迁移exit-0步骤；当前前端另以T45受测实现的174 tests/build和七条独立探针exit0为证据，旧G前端结果不覆盖T45。全页原生桌面200%与动态`prefers-reduced-motion`按§7.4明确未动态验证且不阻塞，不能写成通过。

## 10. 2026-09-09 审查退回与修复验收

需求方已授权修复 Astra 对 `1ef70e5d5fad245d6e38e1472eaa16ffb523aa59..b4750e4ad7a8785e8ff3e965848526436aae94c3` 的审查问题 B01–B10。原始报告为 `.work/c011/review-20260909.md`，最终前端探针为 `.work/c011/probe-task-observation-20260909-105843.stdout.log`。本节只明确既有 AC 的修复验证切片，不增加产品行为、AC 编号、后端合同或后续 change 范围。原报告和失败日志保留；以下“待完成工作”是本节在2026-09-09编写时的状态，不能覆盖后续实际回填，也不能当作新验证结果。
...
### 10.8 当前任务页重连后的收起详情（B16）

T23当前真实浏览器批次中，重启后的列表与独立API/数据库已为failed，用户重新展开仍是running。Astra在`4bd3a71`同实现上运行`probe-closed-detail-reconnect.py`复现：实际展开running→收起→断线/重连且列表failed→用户显式再展开；观测list=failed、detail=running、详情GET总数仍1。根因为连接重建只重置loading详情，ready缓存留存，随后loadTaskDetail的ready分支直接返回。这违反§6详情由正式GET取得及AC-11/15的重连最新状态，不是要求自动刷新全部收起任务。

B16沿AC-11 [外部输入]、AC-15 [并发]：连接/过滤重建后，收起任务保持收起且不自动发详情GET；用户后来显式展开时必须取得重建后当前详情，精确展示status/progress/finished_at/error_msg/cancel_requested_at。缓存由此前显式展开或取消确认读取建立均适用；其他当前展开任务的重建继续成立。不得轮询、重发mutation、按测试ID特殊处理或增加服务器版本/长期缓存。以T45新增独立任务系统mock回归及原真实T23路径验证，原失败保留；其余T44页面/视觉证据未受影响。
