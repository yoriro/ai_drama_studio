...
- [x] 补齐 gen_shots 的失败/取消边界：LLM 或校验失败发生在替换前；待替换结构变化时最终提交失败；running 取消与业务提交只能有一个原子胜方，任何失败不重试且不覆盖先到终态。
  - **前置 task：** T5。
...
- [x] 在现有集工作区剧本页增加“生成分镜”：未保存编辑态不可发起，点击先 impact，非空影响显示准确删除数量并明确确认，0/0 直接提交；202 呈现 task id/任务中心，错误直显 `detail.message`。
  - **前置 task：** T3。
...
- [x] 把集工作区“分镜”标签接入真实 GET/PATCH：按 order 展示只读时长和五类可编辑内容，支持项目 character/scene 绑定，显示 changed、零场景/多场景和旧剧本角标；保留既有返回导航与其他标签行为。
  - **前置 task：** T7、T8。
...
- [x] 在 T1 正式模板验收库从 UI/API 跑通真实 vLLM gen_shots：先复用 C005 已生成资产的同一段剧本逐镜核对；再用含跨地点转场和纯特写的专用剧本核对拆镜与零场景。只记录真实模型输出，不允许后端或测试夹具修补结果。
  - **前置 task：** T5、T10、T11。
