...
| AC-20 | [常规] | 打开并运行三个获窄授权 health 测试及所有新增测试，检查 git diff 与 TRACEABILITY | 测试断言、用例名、diff、追溯行 | 既有三文件只把 hashes 精确集合演进为 zimage+minimaxh3，其他断言逐项保留；其他既有测试零修改；每个新增用例归属追溯行且无恒真/只非空弱断言 |
