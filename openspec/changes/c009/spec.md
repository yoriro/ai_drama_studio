...
| AC-20 | [常规] | 打开并运行三个获窄授权 health 测试、分阶段演进的 C008 contract 测试及所有新增测试，检查 git diff 与 TRACEABILITY | 测试断言、用例名、diff、追溯行 | health 三文件只把 hashes 精确集合演进为 zimage+minimaxh3；C008 contract 用例只按 T9/T13/T15 实际阶段向精确集合加入获授权 path/handler，既有 C008 行为/错误断言逐字保留且不改为子集/存在性；除这四个获授权文件外既有测试零修改；每个新增用例归属追溯行且无恒真/只非空弱断言 |
