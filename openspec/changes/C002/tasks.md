...
- [ ] 把项目创建和修改中的数据库级风格 FK 并发冲突转换为统一 `409 conflict`，保证事务整体回滚；正常的不存在 style_id 仍为 422，其他未预期异常仍传播为 500。
...
- [ ] 在项目详情每个剧集卡片增加明确的“编辑剧本”链接，并在集工作区四个选项卡增加指向当前项目详情的“返回项目”链接。
...
- [ ] 为剧本保存增加明确、可访问且不会陈旧的成功状态：保存期间显示进行中，成功后留在原页显示 API 返回 revision，用户再次编辑或保存失败时清除旧成功消息。
