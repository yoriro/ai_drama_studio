...
- 不新增或修改后端 router、schema、service、task handler、模型、migration、工作流绑定或提示词模板；C010 只消费 C008/C009 已有合同。
...
|---|---|---|---|
| PRD §12.1：MiniMax H3 ref2v API workflow 与 binding | 编写前端与纯逻辑测试不依赖 workflow 现场运行；真实“点击生成→take 可播放”验收前，后端 health 必须报告 `workflow_bindings.status=valid` 且 `hashes.minimaxh3` 为本次现场值 | C009 T17 曾用给定 workflow 完成 1-reference/2-reference 视频；T29 完整回归通过。该证据证明已实现能力，不证明当前 Comfy 进程仍在线 | **不阻塞 C010 文档和 UI 开工**；真实视频浏览器 task 开始前必须刷新 health/queue 证据，失败则停止该 task，不伪造 hash、workflow 或视频 |
...
3. 真实视频验收继续使用 C009 的生产 worker、正式 vLLM/Comfy/workflow/template、PostgreSQL、Task REST/WS 和正式 MP4 媒体通路，没有 C010 专用代理或 fake。它能证明浏览器按钮到可播放 take 和 stale 竞态的完整链路；画面审美、长期一致性和模型确定性不适合自动断言，只人工记录“可播放、有视频流、actual_duration>0、画面非空”及现场截图/日志。
...
5. 除 T10A 的单一一次性 fixture driver 外，本 change 不自建 demo、长期验收 endpoint、第二数据存储或仓库脚本。T11/T12 的页面读取、preview/create、PATCH/DELETE、Task/WS、vLLM/Comfy 与媒体仍走生产通路；依赖 fixture 的证据不得扩张为“上游生成也已验收”。审查阶段若按授权另建 `.work/c010/probe-*.py`，必须说明代理/延迟差异，且不能替代生产浏览器和真实视频证据。
...
| AC-17 | [常规] | 执行 C010 定向测试、完整前端测试/build、完整 pytest、Alembic current/check、范围/追溯/完成报告审计 | 原始命令输出、TRACEABILITY、`.work/c010/completion-report.md`、git | 所有新增测试先有追溯行并回填真实 ID；完整 frontend/backend 通过，Alembic唯一 head且 no new operations；既有测试零修改、backend/migration零 diff；完成报告逐项“操作 → 观测值”覆盖 AC-01..16 与异常分支，未验证项不宣称完成；NOTES/DECISIONS/checkbox/commit一致 |
...
AC-01/AC-17 的范围、文档、Alembic 与完成报告不适合新增单一运行时自动测试；替代验收固定为 git diff/range、OpenAPI 与围栏扫描、`alembic current/check`、完整 pytest、前端 test/build 和人工逐项核对。AC-03/04/13/14 的可确定投影与竞态必须有前端自动测试；AC-02/05-12 的实际控件/DOM、AC-15/16 的生产链路按 §9 的理由采用真实浏览器证据，不以“追溯表无行”跳过。
