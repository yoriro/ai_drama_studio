...
模板按输入顺序输出片段内 [Shot 1] 至 [Shot N]，逐镜独立段落；后续各镜开始时间按 duration_est 比例分配到 requested_duration。summary 与详细动作一致，初始姿态不能替代后续具体动作，动作方向/结果必须写入对应 Shot。原 Shot25 的举臂与指向按先后发生解释，不在数据库改写输入来混淆模板效果。原对白、身份和核心剧情不改；删除模板中强制打斗节奏、笼统自由补全、重复外观及最低字数要求，不增加音频能力。
...
最新授权覆盖上述诊断次数限制：需求方要求“修订模板再生成，直到prompt符合要求为止”。允许对同一冻结输入开展模板修订与真实提示词诊断循环，逐轮保留模板渲染输入、原始响应及逐项审阅；发现缺项后针对通用模板规则修正，不手工改响应、不变更模型/温度/分镜/参考、不放宽验收。此权限只用于本次prompt优化，不改变正式生产任务无自动重试规则，不授权视频抽卡循环或绕过设置API部署。完成以最终原始prompt同时满足结构/绑定/动作/对白为准；单次通过不等于模型稳定性或视频验收通过。
...
2026-09-13 本轮窄修订授权：需求方明确允许为修复当前 Clip1 诊断的比例切点与逐镜对白缺项，且仅允许修改 `backend/deployment/templates/minimaxh3.txt` 的三处通用规则：一，删除会诱导直接复用原累计值的计时示例，明确以 `requested_duration * sum(previous duration_est) / sum(all duration_est)` 求每个累计起点，先用完整精度计算后只对最终起点四舍五入到毫秒并格式化为 `MM:SS.mmm`；二，对每个输入镜头先建立对应段落并按原顺序逐字复制该镜头全部对白，再补充景别、运镜、场景和动作，只移除已识别说话人前缀，空对白不写 speech、空 d 或 silence 占位，保留既有多说话人/无映射声源语义；三，删除重复的 `PARAGRAPH HEADER PATTERNS`，只保留一套与宿舍/球馆无关的三镜完整六段示例，示例使用不等权重且权重总和不同于 requested_duration、第一镜较长对白、第二镜另一人物对白、第三镜空对白，并展示正确计算与逐镜复制。不得把宿舍/球馆数值写入规则，不据此断言错误示例已是已证实根因；五变量、JSON schema、Subject/Picture编号、其他模板、生产代码、迁移、测试、剧本与验收标准不变。

本轮验证按 A→B→C→D 串行：A 先做正式输入校验、renderer 回放、diff 和示例人工核对；B 在确认真实 DB/DATA_DIR、模型参数、GPU 独占、空队列后，使用同一修订模板先对 Clip1 宿舍、再对 Clip2 球馆各执行一次真实诊断，任一失败即停止且不循环修模板；C 仅在两批通过后经正式设置 API 安装并在同库安全重启后逐字回读，旧 T47 部署证据仅作历史；D 仅在此基础上按 Clip2→Clip1 各一次正式页面视频任务取证。旧 T26C Clip1 失败批次与 T22/T23/T26 证据保留，诊断不替代正式 Task/take/current，未授权自动循环或视频重抽。
...
| AC-26 | [外部输入] | 按§6.3对两个冻结片段执行单次提示词诊断与正式新take复验 | 输出详细分镜数量/顺序等于输入、时间点符合比例、各核心动作进入对应镜头且summary一致；部署GET与重启GET等于批准正文、其他模板不变；真实任务与诊断分开保存，视频动作按AC-19逐项记录，失败不重试或伪填通过；本轮修订诊断顺序为Clip1宿舍后Clip2球馆各一次，任一失败即停止 |
