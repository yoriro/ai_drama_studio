...
| AC-26 | [外部输入] | 按§6.3对两个冻结片段执行单次提示词诊断与正式新take复验 | 输出详细分镜数量/顺序等于输入、时间点符合比例、各核心动作进入对应镜头且summary一致；部署GET与重启GET等于批准正文、其他模板不变；真实任务与诊断分开保存，视频动作按AC-19逐项记录，失败不重试或伪填通过 |
