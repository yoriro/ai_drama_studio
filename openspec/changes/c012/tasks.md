...
- [ ] T26C 验证单次模型正文并部署批准模板
  ...
  - 2026-09-13当前修订批次实际失败：使用磁盘修订模板仅执行一次 Clip1 宿舍诊断，命令 `python -X utf8 .work/c012/prompt_diagnostic.py run --clip 1` wrapper exit 0，证据目录 `.work/c012/T26C-clip1-20260913_152658`，实际模板全文另存为同目录 `template.txt`。结构检查 headings/shot labels/time labels/time_axis/english_non_dialogue 均为 true，观察起点 `[3.0,5.5]` 在新合同下有效；两条输入对白均逐字存在。人工逐镜发现 Shot2 输出漏掉输入事实“坐在书桌前”，属于模型漏提，故语义验收失败。Subject/Picture 引用与输入有依据，无数值 existing_id/asset_id/image_id 复用；本批未进入平台 Task/Comfy/合并路径，不能归因后端丢弃。后检 tasks=15/active=0、Comfy=0/0、vLLM sleeping=true，完整审查见同目录 `review.md`。按失败即停，Clip2、修订正文部署及 T26D 未执行，T26C/T26D/T47/T50及T34–T36保持未勾选。
