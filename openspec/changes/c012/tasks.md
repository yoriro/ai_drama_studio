...
- [x] T26C 验证单次模型正文并部署批准模板
  - 2026-09-11最新裁决：需求方接受指定prompt的真实Comfy生成作为T26替代验收，本项原部署路径不再是T26的前置。以下未执行操作及失败记录保留，不冒充已完成部署；候选模板的部署状态仍须在发布收口如实核对。
  - 历史状态（2026-09-13早批，已被当前6570-byte批替代）：需求方授权通用性修正后，球馆 `.work/c012/T26C-clip2-20260911_132008` 与独立小样 `T26-generalization-sample-20260911_131923` 均结构exit0但内容未通过；前者有未翻译词及空对白占位，后者重排引用身份/旁白编号。完整证据与通路见 `.work/c012/T26-generalization-review.md`；该批未部署/生成视频。
  - 历史结果（2026-09-11，已被当前6570-byte批替代）：用户授权循环第11轮球馆prompt经结构及逐镜人工核对通过，原始输出 `.work/c012/T26C-clip2-20260911_122039/prompt.txt`，review.md/checks.json留证，命令exit0。总11轮含1次超时，旧失败保留；该批未执行Clip1模型诊断、设置部署/重启/视频。
  - 2026-09-13历史授权记录：不重新生成以重判已通过的T26；本轮为AC-26补取一次Clip1真实模型诊断，并在T26D按Clip2→Clip1各一次正式平台消费。失败即保留证据并停止后续；旧T47部署证据保留，当前消费状态见本任务末条与T26D。
  ...
  - 2026-09-13历史修订批次 Clip1 诊断：使用磁盘修订模板仅执行一次 `python -X utf8 .work/c012/prompt_diagnostic.py run --clip 1`，wrapper exit 0，证据目录 `.work/c012/T26C-clip1-20260913_152658`，实际模板全文另存为同目录 `template.txt`。结构检查 headings/shot labels/time labels/time_axis/english_non_dialogue 均为 true，观察起点 `[3.0,5.5]` 在新合同下有效；两条输入对白均逐字存在。Astra确认 Shot2 姿态承接不阻塞；其余操作、无ID复用、未进入平台 Task/Comfy/合并路径及终态观察均按原记录保留。该批已被当前6570-byte批替代。
  - 2026-09-13历史修订批次 Clip2 诊断失败：使用同一磁盘修订模板仅执行一次 `python -X utf8 .work/c012/prompt_diagnostic.py run --clip 2`，wrapper exit 1，证据目录 `.work/c012/T26C-clip2-20260913_154409`，实际模板全文另存为同目录 `template.txt`。结构检查 headings/shot labels/time_axis=true，但 `english_non_dialogue=false`，原始未译片段为“两侧设有观众席”；人工逐镜发现 Shot1 漏掉出场方向注视及明确指向、Shot2/Shot4 有 `No dialogue.` 占位。两条对白、3个Subject/Picture映射正确，无ID复用；未进入平台 Task/Comfy/合并路径，不能归因后端丢弃。后检 tasks=15/active=0、Comfy=0/0、vLLM sleeping=true；该批按失败即停，已被当前6570-byte批替代。
  ...
  - 2026-09-13历史五点候选 B 阶段：以 HEAD 当时的 12,221-byte/88-line `minimaxh3.txt` 为基线，候选为 10,146 bytes/79 lines；Clip1通过、Clip2因未译 `两侧` 失败，诊断未进入平台 Task/Comfy/merge；完整快照、diff、review及两次observe证据保留。该候选已被当前6570-byte批替代，不表示当前模板状态。
  ...
  - 2026-09-13空间连接词候选单次复验失败：按授权只在 `backend/deployment/templates/minimaxh3.txt` 增加普通方位词英文原则及词例后，先执行一次 Clip1（`.work/c012/T26C-clip1-20260913_184934`，结构/英文/对白/核心动作人工核对均满足），再执行一次 Clip2（`.work/c012/T26C-clip2-20260913_185321`）。Clip2 原始 stdout 显示 `english_non_dialogue=false`、未译 `两侧`，并在 Shot2/Shot4 输出模板禁止的 `No dialogue` 空对白占位；`response.json`、提取 prompt、请求、模板快照与逐项 review 均保留。外层 wrapper 未在 postamble 前捕获原生退出码，`.work/c012/T26C-spatial-clip2-20260913.exit-code.txt` 明确记录 `NOT_CAPTURED`，不伪造数值；`observe --real` exit 0，目标库/DATA_DIR匹配、tasks=16/active=0、Comfy=0/0、vLLM sleeping=true。参考映射有输入依据，无数值 existing_id/asset_id/image_id 复用；该 prompt-only 批未进入 Task/Comfy/merge，故为模型输出失败，不是后端丢弃。T26C/T47/T26D/T50及T34–T36保持未勾选，T22/T23/T26与旧失败证据保留。
  - 2026-09-13 reference-positive 候选单次复验失败：按 Astra 批准的精简 diff 更新引用描述/空对白正向说明后，`selfcheck` exit 0、`preflight --real` exit 0；仅执行一次冻结 Clip1，目录 `.work/c012/T26C-clip1-20260913_190501`，wrapper native exit 0，结构六项全 true，但输入 shot id 1/2 的两条非空对白在 raw response 与提取 `prompt.txt` 中均缺失，`<d>[Chinese]` 计数为 0，判定模型漏提。生成动作/Subject-Picture映射未见另一个核心失败，未见数字 existing_id/asset_id/image_id 复用；未进入 Task/Comfy/merge，不能归因后端丢弃。`observe --real` exit 0，目标库/DATA_DIR匹配、tasks=16/active=0、Comfy=0/0、vLLM sleeping=true；Clip2、重新部署/重启回读、正式视频均未执行，T26C/T47/T26D/T50及T34–T36保持未勾选，旧 T22/T23/T26 与所有失败证据保留。
  - 2026-09-13压缩候选 Clip1 单次复验失败：按 Astra 批准的两点一致性修正（DIALOGUE前置、metadata/override事实）后，模板为 5,266 bytes/40 行；`selfcheck`、`preflight --real` 均 exit 0。仅执行一次冻结 Clip1，目录 `.work/c012/T26C-clip1-20260913_191416`，wrapper native exit 1；结构六项全 false，raw response/提取 prompt 具体出现 `(asset name)` 形式而非 quoted Picture 绑定、中文 STYLE、standalone English names、中文对白被翻译并附字面 `[Chinese]原对白`、缺规范 Shot/time headers、Shot3 的 `No dialogue`。该失败已在 raw response 中存在，判定模型输出失败；无 numeric existing_id/asset_id/image_id 复用，未进入 Task/Comfy/merge，不能归因后端丢弃。postobserve exit 0，目标库/DATA_DIR匹配、tasks=16/active=0、Comfy=0/0、vLLM sleeping=true；Clip2、部署/重启回读和正式视频未执行，T26C/T47/T26D/T50及T34–T36保持未勾选。
- 2026-09-13当前6570-byte format-example批完成（对应本 task AC-15/16/26）：当前 `backend/deployment/templates/minimaxh3.txt` 为 6,570 UTF-8 bytes/60 lines，SHA-256 `13e271e8b59a1a507f0286c43ae9399bbb3ae9d78aa42a221787e0614055c8d8`。B阶段沿同一冻结输入、同一模型 `Qwen3-30B-A3B-Instruct-2507-AWQ-4bit`、temperature `0.2`、strict `minimaxh3` schema，仅各执行一次 Clip1/Clip2诊断：Clip1 `.work/c012/T26C-clip1-20260913_191926`、Clip2 `.work/c012/T26C-clip2-20260913_192116`，wrapper均exit 0，结构/时间/英文/对白/引用/核心动作逐项核对满足；无数值 existing_id/asset_id/image_id 复用，prompt-only诊断未进入Task/Comfy/merge。C阶段首次 `verify-inputs` 因工作树CRLF exit 1 的原始证据保留；按HEAD blob bytes恢复后 `verify-inputs`、显式 `preflight --real`、正式API install、安装后verify、同库安全重启后verify及API/asyncpg回读均exit 0，四key齐全、正文等于部署输入、均无`[占位]`，目标DB=`ai_drama_studio_c012_m6_20260910`、DATA_DIR=`D:\ai_drama_studio\.work\c012\t21-m6-data`，重启无自动PATCH。B/C证据见 `.work/c012/T26C-clip{1,2}-20260913_19*` 与 `.work/c012/T26C-formal-current-*`；正式视频消费见T26D。此前条目中的“当前/最新”候选均为历史批次，已由本6570-byte批替代。
- [x] T26D 从正式页面生成并核验修订模板的两段视频
...
- 2026-09-13当前D阶段完成：在真实 Director 页面按 Clip2→Clip1 各提交一次，且无模型诊断外调用。Clip2 task `#17`、take `#7`：task `done/progress=1/error=null`，视频10.125s/H.264/960x544，`is_current=false`，旧 take `#4` 仍 current；Clip1 task `#18`、take `#8`：task `done/progress=1/error=null`，视频8.0s/H.264/960x544，`is_current=false`，旧 take `#3` 仍 current。两段 API/独立DB/媒体探针/完整播放均有证据，目标库=`ai_drama_studio_c012_m6_20260910`、DATA_DIR=`D:\ai_drama_studio\.work\c012\t21-m6-data`，项目原4项资产未变、active task=0。Clip2 formal rendered request 与诊断 request 逐字相等；Clip1仅差既有 T31 尾注 `（T31生成中编辑）` 十字，明确记录 exact=false；两段 built prompt、Comfy prompt ID、DB提交证据存在，raw vLLM response未单独持久化。抽帧与UI播放满足当前 AC-19核心视觉门槛：Clip2为球馆、入场/指向/乔彦茜僵住且无额外肢体/主体消失，Clip1为宿舍、芳嘉蔓入门、乔彦茜抬眼后继续吃饭且无额外肢体/主体消失；Clip2未回切Shot3、Clip1有局部环境细节幻觉及非核心`sleeveless dress`措辞，均作为质量限制，不另加门槛。分类均为未观察到模型漏提、错误 existing_id/asset_id/image_id复用或后端丢弃；完整操作→观测见 `.work/c012/T26D-clip{1,2}-formal-review-current-20260913.md` 及同前缀 task/readback/prompt/media/visual/postobserve 证据。
...
- [x] T32 汇总发布操作说明与各层证据边界
  ...
  - 计划测试层级：不新增自动测试。
  - 追溯行：C012 范围与阶段回归及交付一致性。
  - 2026-09-13历史完成记录：更新 `backend/deployment/README.md`，修正历史 2026-09-13 10,756-byte 批的 install/verify/重启回读、显式 DB/DATA_DIR、恢复失败规则、示范媒体入口、R/B/真实 GPU 证据边界与未验证限制；历史 T26 诊断状态均明确标注为历史。只读人工审计 `.work/c012/T32-manual-audit-20260913-corrected.stdout.log` exit 0：历史模板与输入逐字相等（10756 bytes，SHA-256 `2118b57d3472bd6a25247cb0ddad658e01e77a3539faf4963de08d33e569e3fd`）、示范库 project #2/episode #2存在，4项资产、28条分镜、3个Clip、5个视频、15个Task，active Task=0；API health/binding healthy/valid。审计首轮因错误假定 `assets.episode_id` 失败的原始输出与 exit=1保留，修正后未修改生产数据；未新增测试，未运行会触碰vLLM sleep/wake的 `preflight --real`，该项不作为真实GPU验收。当前 README 顶部状态已改由6570-byte批证据覆盖。
...
- [x] T47 关闭最终交付模板与部署门槛（B4）
  ...
  - 依赖（历史批准基线）：既有 T21 证据与 2026-09-13 需求方对 10,756-byte 正文的明确批准仅对应历史基线；当前批次已由 Astra 授权的 6570-byte format-example 候选替代。先核实目标 DB/DATA_DIR、当前任务及进程归属，再按正式 API 安装；与 CPU 线独立。
  - 交付：按当前6570-byte正文及 spec 的执行顺序，经现有正式设置API安装/安装后及同库安全重启后逐字回读，其他三模板不变；复用有效T21新库证据并核对相关输入变化，补当前正文实际消费证据。不得为T26重新生成；当前T26C/D消费证据已经分别回填。
  ...
  - 2026-09-13历史 task #16 批次补证：该正式页面批次实际使用当时的 12,221-byte/88-line 模板，曾完成旧 C阶段 install/verify/同库重启回读，随后仅提交 Clip2 task `#16` 并落入 take `#6`；prompt/媒体实际消费未满足 AC-26，raw vLLM response 未持久化。该批次及其失败证据保留为历史，不作为当前6570-byte部署或T47当前状态。
  - 2026-09-13历史五点候选补证：本候选从 HEAD 前 12,221-byte/88-line tracked 模板出发，工作树为 10,146 bytes/79 lines，尚未经正式 API 安装；selfcheck/preflight exit 0，Clip1 经 Astra 复核通过，Clip2 唯一一次 exit 1（`english_non_dialogue=false`，未译 `两侧`），诊断未进入平台 Task/Comfy/merge。证据见 `.work/c012/T47-template-five-point-20260913.txt`、`.work/c012/T47-template-five-point-diff-20260913.patch`、`.work/c012/T26C-five-point-clip{1,2}-20260913.*` 与对应诊断目录；按失败即停，该候选已被当前6570-byte批替代。

  - 2026-09-13历史空间连接词候选补证：历史工作树模板为 10,385 bytes/79 行、SHA-256 `3bcc78d8eeb55f7dc2e68b94b15ec9cc1db1572b538a4ed9dac4f2680cef8b6c`，相对五点候选仅新增一行；其快照/精简 diff见 `.work/c012/T47-template-spatial-connectors-20260913.txt` 与 `.work/c012/T47-template-spatial-connectors-diff-20260913.patch`。该候选尚未安装运行库；Clip2失败，不以旧正文回读替代当前批次。
  - 2026-09-13历史 reference-positive 候选补证：历史模板为 10,283 bytes/79 行、SHA-256 `ffdf2b930d0f692cb99f4e79af1481fada5294d98a0e4ff821c7632e497b7118`；其 Clip1 因两条非空对白模型漏提失败，未运行 Clip2或部署。精简 diff 与完整失败证据保留；该候选已被当前6570-byte批替代。
  - 2026-09-13历史压缩候选补证：历史模板为 5,266 bytes/40 行、SHA-256 `8a24a51be87ec4b67acb75417a268deb06d4e242efa39ae09ea13679281392fb`；其 Clip1 raw response失败，未执行 Clip2或部署。精简diff与全部失败证据保留；该候选已被当前6570-byte批替代。
- 2026-09-13当前6570-byte交付闭环完成（对应本 task AC-13/15/26）：当前部署输入唯一为 `backend/deployment/templates/minimaxh3.txt` 6570 bytes/60 lines、SHA-256 `13e271e8b59a1a507f0286c43ae9399bbb3ae9d78aa42a221787e0614055c8d8`。已在目标库 `ai_drama_studio_c012_m6_20260910`、DATA_DIR `D:\ai_drama_studio\.work\c012\t21-m6-data` 通过正式设置 API install，安装后 verify 与同库后端安全重启后 verify 均exit 0；API/asyncpg回读四个固定 key齐全，正文逐字等于部署输入、均无`[占位]`，重启无自动PATCH。当前实际消费为T26D task `#17/#18` 与 take `#7/#8`，两者均done、非current且旧current保留；Clip2 rendered request与诊断request逐字相等，Clip1的唯一十字差异为既有T31标记并已记录 exact=false。T26C/T26D的当前证据已分别回填；旧10,756/12,221/10,146/10,385/10,283/5,266批次、task #16及全部失败证据保持历史标记，不作为当前状态。
