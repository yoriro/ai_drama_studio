...
- **C009 对 C007 handler 注册基线的一次性窄演进（需求方于 2026-09-01 授权 Sol 裁决）**：仅允许在 T13 修改 `backend/tests/task_system/test_c007_resource_lifecycle.py::test_pipeline_handler_registration`：保留现有 `gen_asset_image_handler` import 与精确 identity 断言，新增生产 `gen_clip_video_handler` import，并把 `gen_clip_video` 不存在断言替换为该 key 精确 `is gen_clip_video_handler`。不得修改该函数其他内容、该文件其他测试或任何资源顺序/请求断言，不得改为只判 key 存在、skip、改名、删除或使用测试特判/延迟注册；C009 T13 专用提交测试仍须独立证明视频 handler 完整执行与原子提交。本例外在 C009 归档时失效，不得类推。
