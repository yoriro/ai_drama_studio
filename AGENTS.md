...
- 全新数据库只执行 Alembic migration；spec、归档 change、旧验收数据库和设置页历史数据都不会自动成为新库 seed。按 PRD §7，migration 中四个固定 key 继续只放明确的 `[占位]` 示例，不得把正式模板复制进 migration、生产代码常量或隐藏 system prompt。
- `script2assets`、`script2shots`、`zimage`、`minimaxh3` 的正式内容已经分别在 C005、C006、C007、C009 获得需求方确认。C012 必须把这四份既有内容整理为生产部署输入，并在全新生产等价数据库迁移后通过正式设置 API 全部安装；这不是重新创作模板，也不增加 schema。
- C012 发布验收必须在安装后和一次后端重启后分别 GET 回读，精确证明四个 key 齐全、正文与批准输入逐字一致且均不以 `[占位]` 开头，并由 M6 全链路实际消费四份模板。任一缺失、占位或不一致都必须阻塞发布。
- 模板部署与验收不得直接写数据库、复制旧数据库、依赖操作者在 E2E 中临时手改，或用默认值、fallback、旧库结果冒充新库证据。
...
- **C010 MiniMax workflow hash 基线的一次性窄演进（需求方于 2026-09-03 明确授权）**：仅允许在 T11A 将以下四个既有常量中的旧值 `bfa1fbfffecf1665309b01234621bc32cd29f86fd3dfa40f12605cbf3eb3f780` 精确替换为当前授权 workflow 单叶修正后的 Windows checkout raw hash `4f078c121b8ec0d9023e775e0b052036407a5f75bf626d13ea223ebf3d5b4772`：`backend/tests/api/test_c007_health.py::MINIMAX_WORKFLOW_HASH`、`backend/tests/api/test_c009_health.py::MINIMAX_WORKFLOW_HASH`、`backend/tests/api/test_system.py::MINIMAX_WORKFLOW_HASH`、`backend/tests/unit/test_c009_workflow_binding.py::EXPECTED_HASH`。四个文件各只允许这一处字符串常量发生 1 insertion/1 deletion；必须保留全部测试名称、参数、fixture、transport guard、精确响应体、调用次数、binding 路径/只读快照及其他断言，不得动态计算 expected hash、放宽为格式/非空/集合断言、skip、删除、改名或修改任何其他测试。生产代码不得识别测试环境；本例外只同步已授权的 `backend/workflows/minimax_h3_ref2v.json` 节点 `310.inputs.lora_name` 单叶变化，不授权其他 workflow、hash 算法、换行、Python 或配置改动，并在 C010 归档时失效，不得类推。
- **C010 Comfy `execution_error.node_id` 既有测试的一次性窄修正（需求方于 2026-09-03 明确授权）**：仅允许在 T11C 修改 `backend/tests/task_system/test_c009_review_worker_failures.py`：在 `_worker_failure_websocket` 发送的两个 `execution_error.data` 对象及 `test_c009_review_worker_failures_preserve_resources_and_error` 对这两个对象的精确 body 断言中，将四处键名 `"node"` 精确改为 `"node_id"`，值保持 `None`/`"168"` 不变；在 `_assert_failure_error` 中只把末行期望的 `node=168` 精确改为 `node_id=168`。必须保留测试名、参数、其他字段、跨 prompt 忽略、精确操作顺序、wake/chat/sleep/upload/submit/WS/free 次数、无 retry、无 ClipVideo、Task/Clip failed、完整 traceback、资源区间与所有其他断言；不得修改该文件其他字符、任何其他既有测试，或 skip、删除、改名、放宽断言。生产仅允许 `backend/app/tasks/gen_clip_video.py` 把匹配 prompt 的错误节点字段从旧 `node` 改读非空 `node_id` 并在错误末行使用 `node_id=`；不得同时接受旧键、修改 `gen_asset_image`、增加 fallback/history 查询或识别测试环境。本例外在 C010 归档时失效，不得类推。
- **C010 T30 mutation 资源消失回归的一次性窄补充（需求方于 2026-09-06 明确授权）**：仅允许在 T30 修改 C010 新增的 `frontend/src/features/director/directorReviewMutationWiring.test.ts`，为 AC-24 追加一个独立用例及其直接必需的 import、局部 fixture、deferred/flush/FakeSocket 装置；用例必须经生产 `createDirectorMutationAdapter` 与 `createDirectorSync` 发起一条结构化 404 或 409 的真实 action seam，让生产 page reader 的最新 Clip 列表不再包含当前 Clip，并精确断言原 mutation 一次、`detail.message` 原文、正式 page GET 路径与次数、无 mutation 重放、无 success callback/notice、`selectedClipId === null` 且 `clipDetail === null`。T25 既有五类 action×404/409 矩阵的测试名、参数、请求与全部断言必须逐字保留；不得修改生产代码、其他测试文件、skip/删除/改名/放宽任何既有断言、手填 GET/notice 账本或识别测试环境。本例外只补齐经 Sol 探针确认缺失的已规定验收分支，在 C010 归档时失效，不得类推。

## 验收频率与证据复用（需求方于 2026-09-09 授权）

- 每个 task 完成其规定的受影响范围验收后可勾选并提交；不再默认每个 task 运行完整 `pytest`。tasks 必须明确阶段完整回归落在哪一项，change 完成前必须有覆盖最终受测实现的完整回归。
- 纯文档改动检查文档/追溯/commit 一致性及 `git diff --check`，不因文档提交重跑 test/build/pytest。CSS/布局改动运行 build 与受影响页面的真实浏览器检查；前端状态、请求或竞态改动运行对应定向用例、前端 test/build 及规定探针，不自动触发后端全量。
- 后端或验收装置改动运行实际受影响的 API、任务系统 mock、跨进程/资源生命周期检查；涉及数据库、队列、lifespan、公共协议或依赖的广泛影响时提前运行完整回归并说明理由。每个小修复不机械重复全套；风险场景的定向验证不能移到收尾后补。
- 阶段收口及最终交付核对完整回归，包括项目规定的前端 test/build、隔离环境完整 pytest 与迁移检查；审查者收到明确复跑完整 pytest 的指令时仍执行。最终核验可引用同一受测输入的阶段结果，不重复运行同一套件。
- 复用必须列实际受测 commit、当前差异、相关生产/测试/依赖/装置/环境配置、原命令、原始日志及真实退出码；仅文档 commit 不使运行证据失效。相关输入改变后重跑受影响检查，并在收口前更新完整回归证据；失败、未运行或缺原始结果不能复用为通过。不新增 hash、缓存、登记系统或第二套启动器。
- 浏览器功能、并发/取消/快照、跨进程/资源生命周期及其他 AC 专项证据不因全量通过而豁免；不修改测试断言、生产失败不重试规则、数据库隔离或安全边界。当前 task 的必需门槛失败/未运行仍不勾选；尚未到阶段全量不等于本 task 失败，也不得据此宣称 change 完成。
- 报告区分“本次执行”“引用已有证据”“本 task 不要求，待阶段收口”。已有正在运行的验证不为节省时间强制中断；失败保留原证据，先修根因再运行相关检查，不循环跑到绿。
