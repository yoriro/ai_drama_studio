...
### C012 归档补充（2026-09-14）

- C005–C009 是四份模板的原始批准来源；需求方随后授权在 C012 定向修订 `minimaxh3`，不再要求该 key 与 C009 历史正文相同。最终部署输入位于 `backend/deployment/templates/`，其余三个正文未改；正式设置 API 安装、安装后/同库重启后逐字回读、真实生成实际消费的约定不变，不增加模板版本机制。
- C012 已完成这一部署与消费闭环，独立复审为 PASS，受审提交 `bf7e0b2`。来源和限制见 `openspec/archive/c012/spec.md` §6.3、`openspec/archive/c012/review.md` 与 `completion.md`。完成报告中“部署证据闭环”候选并入既有 D-014，不重复建立决定；另两项诊断证据候选保留在报告中，不提升为新的生产机制或普遍验收豁免。

## D-015 辅助页面通过当前 history entry 保存工作页面来源

- 决定：从工作页面进入设置或任务中心时，只在该辅助页面的 Router state 中保存工作 URL 的 pathname、search、hash；辅助页互跳及页内操作继承原来源。返回只接受现有工作路由和正安全整数项目/集 ID，不接受外部地址、辅助页地址或 query 中的跳转授权。合法来源使用 replace 返回；无来源显示明确的返回项目首页入口，非法非空来源显示“返回来源无效”并提供首页入口，不执行非法跳转。
- 理由：来源属于当前标签页的这次导航；使用框架已有 history entry 可保留精确项目、集、选项卡及 query/hash，避免全局最近页串扰与辅助页返回循环。
- 影响：设置、任务中心及以后复用该导航合同的入口。来源不携带业务数据，不恢复草稿、滚动或 Clip 选择，不触发自动保存；目标页面仍通过正式 GET 重建，资源不存在时显示正式错误。不劫持浏览器原生后退，也不把固定目的地返回改为 navigate(-1)。
- 来源：已验收 C011 spec §2.1–§2.3 与来源返回回归/浏览器验收；需求方于 2026-09-10 授权收尾并回填决策。详见 `openspec/archive/c011/spec.md` 与 `openspec/archive/c011/review.md`。
